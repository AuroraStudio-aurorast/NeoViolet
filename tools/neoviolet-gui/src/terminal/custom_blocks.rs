...
pub fn paint_custom_block(
    ...
    let mut paint_quad = |qx: Pixels, qy: Pixels, qw: Pixels, qh: Pixels| {
        let l = px((f32::from(qx) * scale).round() / scale);
        let t = px((f32::from(qy) * scale).round() / scale);
        let r = px((f32::from(qx + qw) * scale).round() / scale);
        let b = px((f32::from(qy + qh) * scale).round() / scale);
