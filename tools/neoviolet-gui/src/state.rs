...
pub struct AppState {
    ...
    /// File paths waiting for a PTY to open them.
    /// Set by `on_open_urls` only: `main.rs` drains them into argv before the
    /// first spawn, `NeoVioletApp::render()` pastes any that arrive later.
